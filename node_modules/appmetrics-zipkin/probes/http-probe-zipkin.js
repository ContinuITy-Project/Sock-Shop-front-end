          ...
          if (traceUrl !== '') {
            ...
            if (hasZipkinHeader(httpReq)) {
              ...
              if (spanId !== undefined) {
                ...
                probeData.traceId = global.tracer.id;
              };
            } else {
              global.tracer.setId(global.tracer.createRootId());
              probeData.traceId = global.tracer.id;
              Request.addZipkinHeaders(args[0], global.tracer.id);
            }

            global.tracer.recordServiceName(serviceName);
            global.tracer.recordRpc(method.toUpperCase());
            global.tracer.recordBinary('http.url', httpReq.headers.host + traceUrl);
            global.tracer.recordBinary('cookie',  httpReq.headers.cookie);
            global.tracer.recordAnnotation(new Annotation.ServerRecv());
            global.tracer.recordAnnotation(new Annotation.LocalAddr(0));
            

            aspect.after(res, 'end', probeData, function(obj, methodName, args, probeData, ret) {
              global.tracer.recordBinary('http.status_code', res.statusCode.toString());
              if(res.req.method == "POST" || res.req.method == "PATCH"){
                var body = res.req.session.lastBody;
                global.tracer.recordBinary('body', JSON.stringify(body));
              }
              global.tracer.recordAnnotation(new Annotation.ServerSend());
